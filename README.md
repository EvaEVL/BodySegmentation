...
Для обучения модели был использован [кегловский датасет](https://www.kaggle.com/datasets/tapakah68/segmentation-full-body-mads-dataset). Датасет представляет набор снимков людей в движении и черно-белых масок.
...
# Визуализация
Всего модель обучалась 220 эпох, но выведем данные о последней сотне.
...
# Заключение
Предсказания модели на валидационных данных, если сравнивать с изображением из интернета, хороши, но можно добиться результатов получше.

Хочется здесь отметить, что я понял, как работает базовые решения задачи на сегментацию и как устроена архитектура модели U-net.  
